...
public class ItemDropping
{
    ...
    public GameObject CreateDroppedItemInstance(object itemToBeDropped, Vector2 mousePosition)
    {
        if (this.droppedItemPrefab == null)
        {
            Debug.LogError("Dropped item prefab is null!");
        ...
        if (itemPickup != null)
        {
            // Setting the correct item based on the type
            if (itemToBeDropped is EquipmentDefinition)
            {
                itemPickup.item = (EquipmentDefinition)itemToBeDropped;
            }
            else if (itemToBeDropped is Item)
            {
                itemPickup.item = (Item)itemToBeDropped; 
            }
