...
public class InventoryDragAndDrop : MonoBehaviour
{
    ...
    {
        if (equipmentToBeDropped is StorageItem)
        {
            EquipmentManager.EquipmentSlot slot = equipmentManager.GetEquipmentSlot(equipmentToBeDropped.slotType);
            if (slot != null && slot.storageContainer != null)
            {
                ...
                foreach (var itemStack in slot.storageContainer.GetItems())
                {
                    itemStacks.Add(new ItemStack(itemStack.Item, itemStack.Quantity));
                }
